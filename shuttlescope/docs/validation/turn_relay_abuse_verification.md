# TURN 踏み台化の実測検証 (2026-08-15)

**問い**: 自宅回線に UDP を開けて TURN を立てた場合、それを内部ネットワークへの
踏み台にされるのを本当に防げるのか。

**答え**: 防げる。ただし**既定設定では実際に踏み台にできる**ことと、
**IPv4 の deny を書いただけでは IPv6 表記で回避される**ことを実測で確認した。
`denied-peer-ip` は「入れておくと良い設定」ではなく、入れないと成立する攻撃を
止める唯一の制御である。

関連: `docs/design/MEDIA_PATH_v1.md` 5.6 節 /
`infra/turn/coturn.conf.example` / `infra/turn/verify_turn_hardening.py`

---

## 検証環境

- coturn 4.6 系 (Alpine Linux 3.20 / WSL2)
- 攻撃側は自作の TURN クライアント (RFC 5766/8656/6156 準拠の生メッセージ)
- 「内部サービス」役: IPv4/IPv6 の UDP を待ち受け、受信内容を記録するプロセス
  - IPv4: `172.24.51.174:9999` (RFC1918)
  - IPv6: `fd00::1:9999` (ULA。dummy インターフェースに付与)
- 攻撃者は **TURN の資格情報を持っている**前提。踏み台化は資格情報を持つ者が
  起こす問題なので、資格情報なしで試しても意味がない。
  現行 `backend/routers/tunnel.py` は Settings の固定 credential をそのまま
  返しており、この前提は現実的

## 結果 (全経路、対照つき)

「deny 無し」で通り「deny 有り」で止まることを、経路ごとに確認した。

| 攻撃経路 | deny 無し | 完全な deny 有り |
|---|---|---|
| IPv4 RFC1918 / CreatePermission | **到達** | 403 Forbidden IP |
| IPv4 RFC1918 / ChannelBind | **到達** | 403 Forbidden IP |
| **IPv4-mapped IPv6 (`::ffff:172.24.51.174`) / 両経路** | **到達** | 403 Forbidden IP |
| **IPv6 ULA (`fd00::1`) / 両経路** | **到達** | 403 Forbidden IP |
| IPv6 link-local (`fe80::1`) | **到達** | 403 Forbidden IP |
| ブロードキャスト `255.255.255.255` / 両経路 | **到達** | 403 Forbidden IP |
| `0.0.0.0` | 403 (既定で拒否) | 403 |
| IPv6 loopback `::1` | 403 (既定で拒否) | 403 |
| TURN 自身 | 403 (既定で拒否) | 403 |
| TCP リレー (RFC 6062) | 400 (この構成では無効) | 同左 |
| 認証なし Allocate | 401 Unauthorized | 401 |

最終確認では、脆弱構成で **16 経路が中継可能** (終了コード 1)、
全表記を塞いだ構成で **全経路拒否** (終了コード 0)。
経路の内訳は下の「別表記による回避」を参照。

## 無効だった検証を 2 つ記録する

**攻撃が成立することを先に示せない検証は、防御の証明にならない。**
以下はどちらも「防御が効いた」ように見えて、実際は別の理由で止まっていた。

1. **peer に `127.0.0.1` を使った回**
   防御なし構成でも 403 になった。coturn は loopback を既定で拒否する
   (`--allow-loopback-peers` が opt-in) ため、こちらの設定とは無関係に
   止まっていた。RFC1918 に変えて測り直した。

2. **IPv6 の peer を IPv4 の割当で試した回**
   全て `443 Peer Address Family Mismatch` になった。これは deny が
   効いたのではなく、割当の address family が違うので**検査に到達して
   いなかった**。RFC 6156 の `REQUESTED-ADDRESS-FAMILY` で IPv6 の割当を
   要求してからやり直したところ、**deny 無しでは到達した**。

このため `verify_turn_hardening.py` は 443 を「安全」ではなく
**「未検証」**として扱い、未検証が残ったら終了コード 1 を返す。

## 別表記による回避 (codex の指摘を受けて実測)

denylist は**表記ごとに書かないと効かない**。IPv4 を IPv6 の中に埋め込む方式は
複数あり、`::ffff:` を塞いだだけでは残りが素通りする。

| 表記 | `::ffff:` のみ | 全表記を塞いだ設定 |
|---|---|---|
| `::ffff:172.24.51.174` (IPv4-mapped) | 403 | 403 |
| `::172.24.51.174` (IPv4-compatible) | 403 (coturn が正規化) | 403 |
| `2002:ac18:33ae::1` (6to4) | **素通り** | 403 |
| `64:ff9b::ac18:33ae` (NAT64 WKP) | **素通り** | 403 |
| `64:ff9b:1::ac18:33ae` (NAT64 /48) | **素通り** | 403 |
| XOR-PEER-ADDRESS を 2 つ入れ順序反転 | 403 (両順序) | 403 |

6to4 と NAT64 はこの環境では変換経路が無いため実配送されなかったが、
**ACL が拒否していない**時点で、NAT64 のある環境では成立する。
Teredo (`2001::/32`) も同種なので併せて禁止した。

検証スクリプトを 3 つの設定に当てた結果:

| 設定 | 中継できた経路 |
|---|---|
| 既定 | **16** |
| `::ffff:` までを塞いだ設定 (初版) | **6** |
| 全表記を塞いだ設定 (現行) | **0** |

## 設定の罠 2 件 (実測。どちらも「正しく書いたつもり」を無効化する)

### 罠 1: `allowed-peer-ip` は `denied-peer-ip` に優先する

全表記を塞いだ設定に `allowed-peer-ip=<LAN の IP>` を **1 行足しただけ**で、
内部サービスへ到達した。記述順は関係ない。

deny を何行積んでも allow 1 行で無効化される。**設定ファイルに
allowed-peer-ip を書かない**、が唯一の運用ルールになる。

### 罠 2: CIDR 記法は無効で、しかも fail-open

`denied-peer-ip=172.16.0.0/12` と書くと coturn 4.6.2 は

```
error resolving '172.16.0.0/12' hostname: Try again
ERROR: Wrong address format: 172.16.0.0/12
```

とログに出す。**しかし起動は続き、そのレンジは無防備のままになる。**
CIDR だけで LAN を守る設定にして実測したところ、中継が成立した。

正しい書式は `開始IP-終了IP`。見た目には正しそうな設定が、
ログを読まない限り無防備であると分からない。
**設定の字面ではなく実挙動を確認する必要がある**ことの、最も分かりやすい例。

## 発見した欠陥 (このリポジトリの成果物)

ひな型を 2 度直している。どちらも「書いただけでは分からず、攻撃してみて
初めて分かった」類の穴である。

1. **初版**: `denied-peer-ip=::ffff:0.0.0.0-::ffff:255.255.255.255` が無く、
   IPv6 割当 + `::ffff:` 表記で素通りした
2. **第2版**: 6to4 / NAT64 / Teredo / IPv4-compatible が抜けていた。
   検証スクリプトを当てたところ **6 経路が残っていた**

denylist 方式である限り、表記の網羅性が防御の強さを決める。
新しい埋め込み方式が出れば同じことが起きる。

## ここから言えること

1. **既定の coturn は踏み台になる。** LAN 上の PostgreSQL、ルータ管理画面、
   プリンタ、IoT 機器に UDP で到達できる
2. **経路は 1 つではない。** CreatePermission / ChannelBind / IPv6 表記 /
   IPv4-mapped / ブロードキャストと、少なくとも 5 系統ある。
   1 つ塞いで安心する発想自体が危険
3. **denylist 方式の宿命として、表記の網羅性が防御の強さを決める。**
   新しい表記が見つかれば穴になる
4. TCP リレーはこの構成では無効だったが、**明示的に `no-tcp-relay` を書く**

## 本番ホストの実測 (2026-08-15、read-only)

未検証だった項目のうち 2 つを本番で確認した。

**IPv6 は存在しない。** グローバル IPv6 アドレスは 0 件、`api6.ipify.org` へも
到達しない。したがって「IPv4 のポート転送とは別に IPv6 で直接到達される」
という懸念は**現時点では当たらない**。同時に、coturn が IPv6 のリレーを
確保できないので `::ffff:` 回避も現状では成立しない。

ただしこれは**脆い性質**で、ISP が IPv6 を有効化した日に成立するようになる。
`coturn.conf.example` の IPv6 deny は今は効かないが、外さないこと。

**アドレス構成**
- LAN: `192.168.1.4` (イーサネット)。`denied-peer-ip=192.168.0.0-192.168.255.255` の範囲内
- WSL: `192.168.16.1` (同上でカバー)
- 公開 IPv4: 1 つ

**PostgreSQL は loopback のみで待ち受けている** (`127.0.0.1:5432` / `::1:5432`)。
LAN アドレスにはバインドしていないので、`192.168.1.4:5432` へ中継されても
届かない。**DB を直接潰す経路は、loopback へ中継できた場合に限られる。**
coturn は loopback を既定で拒否し、`denied-peer-ip=127.0.0.0/8` がそれを補強する。
`allow-loopback-peers` を書いた瞬間にこの防御が消えるので、絶対に書かない。

**一方、LAN アドレスで待ち受けているものは踏み台の実害になる**:

| ポート | 内容 |
|---|---|
| 445 | SMB |
| 3389 | RDP |
| 135 | RPC |
| 22 | SSH |
| 8765 | backend API |
| 80 / 8080 / 3434 ほか | 各種 |

`denied-peer-ip` の 192.168.0.0/16 がこれらを覆っている。**この 1 行が
落ちると SMB と RDP に UDP で到達される**ということであり、
deny リストが「あると良い設定」ではないことがここでも確認できる。

Windows Firewall は 3 プロファイルとも有効 (受信の既定動作は未構成 = 既定のブロック)。

## 未検証 (正直に残す)

- **TURN を実際に立てての本番検証は未実施。** UDP を開ける前に
  `verify_turn_hardening.py` を本番へ向けて走らせ、終了コード 0 を確認すること
- ルータ側のポート転送設定と、ルータ自身の管理画面が LAN から
  到達可能かは未確認
- 帯域の踏み倒し (credential を拾われて中継に使われる) は `denied-peer-ip` では
  防げない。短命 credential と quota で対処する
- coturn の既知 CVE に対する最低バージョンの精査は未実施。
  検証に使ったのは **coturn 4.6.2** (Alpine 3.20)。
  なお codex は「4.8.x / 4.13.0 / 4.15 / 4.17.2」といった版数を挙げたが、
  coturn の実在バージョンは 4.5〜4.7 系であり、**この版数指定は採用しない**
- 事前認証 (認証前に処理されるパケットの解析) の攻撃面は未評価

## この検証で守れないもの (codex の指摘。同意する)

`denied-peer-ip` が守るのは **TURN を中継として悪用されること**であって、
**coturn プロセス自体が乗っ取られること**ではない。

認証前のパケット解析に欠陥があれば、攻撃者は coturn として任意のコードを
動かせる。その時点で `denied-peer-ip` は無関係になる。coturn が本番ホストに
同居していれば、通常の TCP クライアントとして `127.0.0.1:5432` へ接続でき、
PostgreSQL が loopback 限定であることも防御にならない。
`.env` が同じユーザーで読めれば DB 資格情報もそのまま得られる。

したがって **「同一ホストに coturn を置く」構成は、ACL がどれだけ完璧でも
本番 DB を守れない**。守るなら別ホスト (VPS / VM + 上流 ACL) へ出すか、
そもそも自前で立てないか。

## 運用への反映

- `infra/turn/coturn.conf.example` — 実測で有効性を確認した設定。
  `use-auth-secret` の時刻ベース一時 credential を既定にし、静的 `user=` を使わない
- `infra/turn/verify_turn_hardening.py` — 同じ攻撃を稼働中サーバへ仕掛ける。
  中継されたら 1、未検証が残っても 1。**既定構成で 16 件、初版ひな型で 6 件を
  検出し、現行ひな型で 0 件になることを確認済み**
- 未修正の関連欠陥: `backend/routers/tunnel.py` が固定 TURN credential を
  そのまま返している。短命 HMAC 方式へ変える必要がある
