...
from backend.routers import insights_summary as insights_summary_router
...
class PlayerAccessControlMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: StarletteRequest, call_next):
        ...
        # ── coach / analyst: クエリ ?player_id= に対する team scope 強制 ──
        ...
        # R283: analyst も同様に他チーム player_id で /api/analysis/* (heatmap /
        # descriptive / shot_types 等) の戦術データを取得できる cross-team leak を
        # live 実証したため、同じ team scope を analyst にも適用する。player_id は
        # 解析対象 = 自チーム選手であり、対戦相手スカウトは opponent_id 経由で別途
        # 許可されるためここでは遮断されない。
        # 互換性: team 未設定の analyst は check_export_player_scope と同様に移行期
        # 扱いで通す (coach は従来どおり team 未設定なら 403)。
        if role in ("coach", "analyst") and not _demo_read:
...
_GLOBAL_AUTH_EXEMPT = _re_acl.compile(
    ...
    # heartbeat / ice-config: JWT を持たない参加端末 (QR + セッションパスワード)
    # が participant_token で呼ぶ経路。単なる免除ではなく、どちらのハンドラも
    # participant_token を検証する (ws-ticket と同じ方式)。
    # participant_id は数値に限定し、末尾もこの 2 route だけに固定する。
    r"|sessions/[A-Za-z0-9_-]{1,32}/devices/\d{1,12}/(?:heartbeat|ice-config)(?:\?.*)?$"
...
app.include_router(insights_summary_router.router, prefix="/api")
