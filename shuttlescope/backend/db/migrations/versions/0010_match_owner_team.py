...
def upgrade() -> None:
    ...
    if "owner_team_id" not in cols:
        op.add_column("matches", sa.Column("owner_team_id", sa.Integer(), sa.ForeignKey("teams.id"), nullable=True))
    if "is_public_pool" not in cols:
        op.add_column("matches", sa.Column("is_public_pool", sa.Boolean(), nullable=False, server_default=sa.text("0")))
    if "home_team_id" not in cols:
        op.add_column("matches", sa.Column("home_team_id", sa.Integer(), sa.ForeignKey("teams.id"), nullable=True))
    if "away_team_id" not in cols:
        op.add_column("matches", sa.Column("away_team_id", sa.Integer(), sa.ForeignKey("teams.id"), nullable=True))
