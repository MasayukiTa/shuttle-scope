...
ゾーン座標マップについて (A-1b / 2026-09-22 に作り直した):
  各ゾーンの重心を **半面** の正規化座標 [0,1]×[0,1] で定義する。
  y=0 がネット際、y=1 がベースライン側。x は画面左=0、画面右=1。

  正本は `src/components/court/CourtDiagram.tsx` の矩形定義で、
  半面を縦 3 等分・横 3 等分している。したがって重心は:
    行 N (Net):   y = 1/6 ≈ 0.1667
    行 M (Mid):   y = 3/6 = 0.5
    行 B (Back):  y = 5/6 ≈ 0.8333
    列 L / C / R: x = 1/6, 3/6, 5/6

  以前はここを `backend/tracknet/zone_mapper.py` の閾値
  (Y_BACK=0.50 / Y_NET=0.25) から導いていた。あれは画像の生座標を
  切るための値で、**B だけがコートの半分を占める不等分**だった
  (B の重心 y=0.750 / M=0.375 / N=0.125)。コート上のゾーンは等分なので、
  ゾーン間距離＝OT のコスト行列が歪んでいた。
...
# 半面を 3x3 等分したときの各ゾーン重心 (x, y)。
# キー: ゾーンラベル (Stroke.land_zone と同じ 2 文字コード)
# 値: (x, y) タプル、[0,1]×[0,1]。y=0 がネット際。
# 手で書いた表を置くと等分でない値が紛れ込むので、等分から導出する。
_ROW_CENTROID = {"N": 1 / 6, "M": 3 / 6, "B": 5 / 6}
_COL_CENTROID = {"L": 1 / 6, "C": 3 / 6, "R": 5 / 6}
_ZONE_CENTROID_MAP: dict[str, tuple[float, float]] = {
    f"{row}{col}": (_COL_CENTROID[col], _ROW_CENTROID[row])
    for row in ("B", "M", "N")
    for col in ("L", "C", "R")
}
