 ...
 * 配信URLダウンロードパネル
 *
 * YouTube / Twitter / Bilibili / ニコニコ動画 など、Electron で直接再生できない
 * 配信URLを検出したときに表示する。yt-dlp でダウンロードし、完了後は
 * localfile:// 経由でローカル再生に自動切替する。
 */
import { useState, useEffect, useCallback } from 'react'
import { Download, AlertCircle, CheckCircle, Loader2, Film } from 'lucide-react'
...
export function StreamingDownloadPanel({
  ...
  // ── ダウンロード進捗ポーリング ──
  useEffect(() => {
    if (!jobId || (dlState !== 'downloading' && dlState !== 'processing')) return
      ...
      try {
        ...
        switch (d.status) {
          ...
          case 'processing':
            ...
            break
          // pending / unknown はそのまま待機
        }
      } catch {
        // ネットワークエラーはスキップして次のポーリングへ
      }
    }, 1500)
  ...
  return (
    <div className="flex flex-col gap-3 bg-gray-800 rounded-lg overflow-hidden" style={{ aspectRatio: '16/9', justifyContent: 'center', padding: '24px' }}>
      {/* ── サービス名 + URL ── */}
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs text-gray-400">
            <div className="flex items-center gap-1.5">
              ...
              <span>
                {dlState === 'starting' ? '開始中...' :
                 dlState === 'processing' ? 'マージ中...' :
                 progress.percent}
              </span>
            </div>
      ...
      {/* ── 品質選択 + ボタン（idle / error 時） ── */}
      {(dlState === 'idle' || dlState === 'error') && (
        <div className="flex items-center gap-2 mt-1">
          <select
            value={quality}
            onChange={(e) => setQuality(e.target.value)}
            className="flex-1 bg-gray-700 border border-gray-600 text-gray-200 text-sm rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white rounded text-sm font-medium whitespace-nowrap transition-colors"
          >
            <Download size={14} />
            ダウンロードして再生
          </button>
