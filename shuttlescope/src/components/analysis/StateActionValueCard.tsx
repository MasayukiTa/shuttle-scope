...
export function StateActionValueCard({ playerId, filters }: Props) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGet<{ success: boolean; data: BestActionRow[]; meta: Meta }>(
        '/analysis/state_best_actions',
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-gray-200">状態-行動価値（Q値）最善手</h3>
        <EvidenceBadge
          tier="research"
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={i} className="border-b border-gray-700/40 hover:bg-gray-700/20">
                  ...
                  <td className="py-1.5 pr-2 text-white font-medium">{row.best_action}</td>
                  <td className="py-1.5 pr-2 text-right">
                    <span className={row.best_q > 0 ? 'text-emerald-400' : 'text-orange-400'}>
      ...
      <p className="text-[10px] text-gray-600">
        Q値 = 状態内最善ショット勝率 - 状態ベースライン勝率。CI幅が大きい場合は統計的根拠が弱いです。
      </p>
    </div>
  )
}
