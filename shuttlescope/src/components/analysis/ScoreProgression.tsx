...
interface ScoreProgressionProps {
   ...
   * 第6引数に RallyPoint を渡すので SetIntervalSummary 内のラリー詳細表示に使える。
   */
  onSetPointClick?: (setId: number, setNum: number, rallyNum: number, scoreA: number, scoreB: number, rally: RallyPoint, setRallies: RallyPoint[]) => void
  /** セット選択の初期値 */
  initialSet?: number
}
...
// ─── ラリー詳細バナー（SetIntervalSummary 内で表示）────────────────────────────

export function RallyDetailBanner({
  rally,
  onClose,
  onPrev,
  onNext,
            ...
            {strokes.map((s, i) => {
              ...
              return (
                <span key={s.stroke_num} className="flex items-center gap-1 shrink-0">
                  ...
                  >
                    {isA ? 'A' : 'B'} {shotLabel}
                    {s.hit_zone && <span className="text-[10px] opacity-70 ml-0.5">{s.hit_zone}</span>}
                  </span>
      ...
      {chartData.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart
            ...
            style={{ cursor: onSetPointClick ? 'pointer' : 'default' }}
            onClick={(chart) => {
              ...
              if (currentSet?.set_id != null) {
                onSetPointClick(currentSet.set_id, currentSet.set_num, point.rally_num, point.score_a, point.score_b, point, currentSet.rallies)
              }
            }}
          >
