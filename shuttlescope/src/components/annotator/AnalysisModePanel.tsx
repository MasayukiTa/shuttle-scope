  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="px-3 py-3 space-y-4 text-xs">
        {/* スコア + コンテキスト */}
        <section>
          <div className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">現在</div>
        ...
        <section>
          <div className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">
            直近 5 ラリー (Player A 勝率 {last5.length ? Math.round((aWins / last5.length) * 100) : 0}%)
          </div>
          <div className="flex items-center gap-1">
            {last5.length === 0 && <span className="text-gray-600">データなし</span>}
            {last5.map((r, i) => (
              <span
                key={i}
                className={
        ...
        <section>
          <div className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">
            現ラリーのショット種別 ({recentStrokes.length}本)
          </div>
          {shotCounts.length === 0 ? (
        ...
        <p className="text-[10px] text-gray-600">
          ※ より詳細な解析 (EPV / momentum / 勝率予測) は Dashboard の Live ページに集約。
        </p>
      </div>
    </div>
  )
