...
export function DashboardGrowthPage({ playerId, filters, sortedPlayers }: Props) {
  ...
  const selectClass = isLight
    ? 'bg-white border border-gray-300 text-gray-700 text-xs rounded-ss-md px-2 py-1'
    : 'bg-gray-700 border border-gray-600 text-white text-xs rounded-ss-md px-2 py-1'
  ...
  return (
    <div className="space-y-5">
      ...
      {sortedPlayers.filter((p) => p.is_target).length >= 2 && (
        <div className={`flex items-center gap-3 ${card} rounded-ss-lg px-4 py-3 shadow-card`}>
          <span className={`text-xs ${textSecondary}`}>{t('analysis.growth.pair_mode')}</span>
          <button
            onClick={() => { setPairMode((v) => !v); setPartnerPlayerId(null) }}
            className={`relative w-10 h-5 rounded-full transition-colors duration-fast ease-out ${toggleClass}`}
          >
      ...
      {/* 成長判定カード */}
      <ErrorBoundary>
        <div className={`${card} rounded-ss-lg p-4 shadow-card`}>
      ...
      {/* 勝率推移 */}
      <ErrorBoundary>
        <div className={`${card} rounded-ss-lg p-4 shadow-card`} data-tutorial="dashboard.growthTimeline">
