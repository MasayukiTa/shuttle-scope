...
function Ensure-RayHead {
  # Ray head が動いていなければ RayHead Scheduled Task をトリガーする。
  # 重要: 直接 `ray start` してはいけない (SSH/job-object 連動 kill / 過去8回再現)。
  # 必ず Scheduled Task 経由 (SYSTEM, detached) で起動する。
  try {
    $up = Get-NetTCPConnection -LocalPort 6379 -State Listen -ErrorAction SilentlyContinue
    if ($up) { Log-Sv 'ray head already up (6379) - skip'; return }
    $task = Get-ScheduledTask -TaskName 'RayHead' -ErrorAction SilentlyContinue
    if (-not $task) { Log-Sv 'RayHead task not found - skip'; return }
    Log-Sv 'ray head down - triggering RayHead scheduled task'
    Start-ScheduledTask -TaskName 'RayHead'
  } catch {
    Log-Sv ('Ensure-RayHead error: ' + $_.Exception.Message)
  }
}
