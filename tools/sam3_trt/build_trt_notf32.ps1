...
$plan = "$bench\sam3_enc_518_fix_notf32.plan"
& "C:\TensorRT\TensorRT-10.16.1.11\bin\trtexec.exe" --onnx=$onnx --saveEngine=$plan --noTF32 --memPoolSize=workspace:4096 2>&1 | Select-Object -Last 25
if (Test-Path $plan) {
    $mb = [math]::Round((Get-Item $plan).Length / 1MB, 1)
    Write-Output "PLAN_BUILT_MB $mb"
} else { Write-Output "PLAN_MISSING" }
