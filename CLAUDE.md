...
- **Zone9 の 2 つの軸は基準が違う（A-1b / 2026-09-22 にここで統一した）。**
  正本は `src/components/court/CourtDiagram.tsx` の矩形定義:
  - **行 (B/M/N) はネット基準** — どちらの半面でも N がネット際、B がベースライン側
  - **列 (L/C/R) は画面基準** — どちらの半面でも画面左が L。**鏡映しない**

- 合成タブの畳み込みは「可視化専用」。相手半面を自半面に重ねるのは
  ネット線での鏡映で、上の基準では **行も列も変わらない（恒等写像）**。
  導出は `backend/utils/zone9_fold.py`。`backend/analysis/` 以下の空間分析コードは
  この畳み込みを呼び出してはならない。空間分析は `hit_zone`/`land_zone` の
  生データのみを使用すること。

- **以前ここには 180 度の点対称表 `ZONE_ROTATION_MAP` (`BL→NR` 等) があった。**
  あれは「列が選手基準」を前提にした表で、このアプリの入力 UI とは前提が違う。
  相手のベースライン際に落ちた球が自コートのネット際に描かれていた。
  「変換が要るはず」という直感で点対称を書き戻さないこと。

- 合成ビューのデータソースは `/api/analysis/heatmap/composite` のみ。
  空間分析のエンドポイント（`/api/analysis/spatial/*`）は独立して動作する。

- テスト: `backend/tests/test_heatmap_composite.py` は **`CourtDiagram.tsx` を読んで
  実際の矩形座標から**畳み込みを検証する。旧テストは変換表をテスト側に書き写して
  自分自身と突合していただけで、座標系が誤っていても絶対に落ちなかった。

- 18 ゾーン側 (`pixel_to_court_zone`) の `depth` は **ネットからの距離**。
  A 側 (コート y<0.5) は y=0 がベースラインなので `depth` を反転して求める。
  2026-09-22 まで A 側だけ front と back が入れ替わっており、同じ "front" が
  半面によってコートの反対の端を指していた。
